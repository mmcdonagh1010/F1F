...
export default function LeaderboardPage() {
  ...
  const [boardMode, setBoardMode] = useState("racePoints");
  ...
  async function loadBoard(targetYear) {
    try {
      const [seasonData, latestData] = await Promise.all([
        apiFetch(`/leaderboard/season?year=${targetYear}${leagueId ? `&leagueId=${leagueId}` : ""}`),
        apiFetch(`/leaderboard/latest?year=${targetYear}${leagueId ? `&leagueId=${leagueId}` : ""}`)
      ]);

      setRows(seasonData.rows || []);
      setRaces(seasonData.races || []);
      setAvailableYears(seasonData.availableYears || []);
      setAvailableLeagues(seasonData.availableLeagues || latestData.availableLeagues || []);
      if (!leagueId && seasonData.leagueId) {
        ...
        <label className="block text-sm text-slate-200">
          ...
          >
            {(availableYears.length > 0 ? availableYears : [currentYear]).map((y) => (
              <option key={y} value={String(y)} className="bg-track-900 text-white">
                {y}
              </option>
            ))}
          </select>
          ...
          <tbody>
            {rows.map((row, idx) => (
              <tr key={row.id} className="border-b border-white/10 last:border-0">
                ...
                <td className="px-2 py-3 font-bold text-accent-gold">
                  {row.totalPoints}
                </td>
                {racesToDisplay.map((race) => (
